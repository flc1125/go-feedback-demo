...
// arModel is a active record design model for table message operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of message.
	Table = "message"
	// Model is the model object of message.
	Model = &arModel{g.DB("default").Model(Table).Safe()}
	// Columns defines and stores column names for table message.
	Columns = struct {
		Id         string //           
        Uid        string // 用户ID    
        Message    string // 留言内容  
